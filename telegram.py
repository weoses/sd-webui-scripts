...
def negative_handler(message:types.Message):
    try:
        ...
        bot.send_message(message.chat.id, reply_to_message_id=message.id, text=msg_conf_node["current_steps"].format(steps=conf_node["steps"]))
    except Exception as e:
        __logFatal(e, message.chat.id, message.id)
...
def steps_handler(message:types.Message):
    try:
        ...
        bot.send_message(message.chat.id, reply_to_message_id=message.id, text=msg_conf_node["current_negative_prompt"].format(prompt=conf_node["negative"]))
    except Exception as e:
        __logFatal(e, message.chat.id, message.id)
...
def model_handler(message:types.Message):
    try:
        ...
        bot.send_message(message.chat.id, reply_to_message_id=message.id, text=msg_conf_node["current_model"].format(model=opts.model))
    except Exception as e:
        __logFatal(e, message.chat.id, message.id)
...
def progress_preview_setting_handler(message:types.Message):
    try:
        ...
        bot.send_message(message.chat.id, 
                         reply_to_message_id=message.id, 
                         text=msg_conf_node["current_show_progress_preview"].format(show_progress_preview=conf_node["show_progress_preview"]))
    except Exception as e:
        __logFatal(e, message.chat.id, message.id)
...
def progress_setting_handler(message:types.Message):
    try:
        ...
        bot.send_message(message.chat.id, reply_to_message_id=message.id, text=msg_conf_node["current_show_progress"].format(show_progress=conf_node["show_progress"]))
    except Exception as e:
        __logFatal(e, message.chat.id, message.id)
...
def __send_waiting(message:types.Message) -> types.Message:
    ...
    if not loading_image_id:
        with open(f"./res/loading.png", "rb") as ph:
            sent_msg = bot.send_photo(photo=ph, chat_id=message.chat.id, reply_to_message_id=message.id, caption=msg_conf_node["working"])
            if sent_msg.photo:
                loading_image_id = sent_msg.photo[0].file_id
    else:
        sent_msg = bot.send_photo(photo=loading_image_id, chat_id=message.chat.id, reply_to_message_id=message.id, caption=msg_conf_node["working"])
...
def __generate(prompt_user, sent:types.Message) -> api.Base64Img:
    ...
    def gen_func(): 
        img = api.gen_img(url, prompt_user, conf_node["negative"], conf_node["width"], conf_node["height"], steps=conf_node["steps"])
        ret.append(img)
